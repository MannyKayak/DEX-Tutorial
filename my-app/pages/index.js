...
export default function Home() {
    ...
    // globals to show in the interface
    const [ethBalance, setEtherBalance] = useState(zero);
    const [reservedCD, setReservedCD] = useState(zero);
    const [etherBalanceContract, setEtherBalanceContract] = useState(zero);
    const [cdBalance, setCDBalance] = useState(zero);
    const [lpBalance, setLPBalance] = useState(zero);
    // globals to check the presence of liquidity
    const [addEther, setAddEther] = useState(zero);
    const [addCDTokens, setAddCDTokens] = useState(zero);
    // globals to keep track of CD and eth to send back to the user and LP tokens
    const [removeEther, setRemoveEther] = useState(zero);
    const [removeCD, setRemoveCD] = useState(zero);
    const [removeLPTokens, setRemoveLPTokens] = useState("0");
    // globals for the swap fuctions: swap amount and if ethers are selected or not
    ...
    // getAmounts: call various functions to retrive amounts 
    ...
    const _addLiquidity = async () => {
        try {
            ...
            if (!addCDTokens.eq(zero) && !addEtherWei.eq(zero)) {
                ...
                setLoading(false);
                // reinitialize the CD amount and call getAmounts to update UI
                setAddCDTokens(zero);
                await getAmounts();
            }
        } catch (err) {
